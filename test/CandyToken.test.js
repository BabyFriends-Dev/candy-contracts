...
contract("CandyToken", ([creator, receiver, thirdParty]) => {
  ...
  const transferAmount = new BigNumber("100e18"); // 100 CANDY tokens
  const approveAmount = new BigNumber("10e18"); // 10 CANDY tokens
  ...
  describe("ERC20::Standard Token", () => {
    ...
    it("assigns the initial total supply to the creator", async () => {
      ...
      const logs = decodeLogs(receipt.logs, CandyToken, this.token.address);
