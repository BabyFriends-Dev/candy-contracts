...
const Airdrop = artifacts.require("CandyAirdrop");
...
contract("Airdrop", ([creator, addr1, addr2, addr3, addr4, addr5]) => {
  const airdropFund = new BigNumber("10000e18"); // 10000 CANDY tokens
  const withdrawalAmount = new BigNumber("100e18"); // 100 CANDY tokens
  ...
  describe("Airdrop::functions", () => {
    it("sets token to be airdropped", async () => {
      let newToken = await CandyToken.new(
        NAME,
        SYMBOL,
        DECIMALS,
        INITIAL_SUPPLY
