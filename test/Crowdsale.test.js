...
const CandyCrowdsale = artifacts.require("CandyCrowdsale");
...
contract(
  ...
  ([
    ...
    const EXCHANGE_RATE = new BigNumber(1000); // 1 ether = 1000 CANDY
    const TOTAL_SALE_CAP = ether(10);
    const VESTING_TOKEN_AMOUNT = new BigNumber("4e22"); // 40,000 CANDY
    const BONUS_RATE = new BigNumber(2500); // 25% Bonus rate which is multiplied by 100 for better resolution

    // These values are for testing purposes. The values set in CandyCrowdsale
    ...
    const vestAmount_funder = new BigNumber("2.5e21"); // 2,500 CANDY
    const vestAmount_thirdParty = new BigNumber("1e21"); // 1,000 CANDY
    const vestAmount_thirdPartyAlt = new BigNumber("5e21"); // 5,000 CANDY

    beforeEach(async () => {
      this.token = await CandyToken.new(NAME, SYMBOL, DECIMALS, INITIAL_SUPPLY);
      this.crowdsale = await CandyCrowdsale.new(
        EXCHANGE_RATE,
        wallet,
        this.token.address,
        TOTAL_SALE_CAP
    ...
    describe("Crowdsale::Lockable", () => {
      ...
      const lockedAmount = new BigNumber("200e18"); // 200 CANDY tokens
